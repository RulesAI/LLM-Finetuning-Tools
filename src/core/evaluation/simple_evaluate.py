...
def is_similar(q1, q2, threshold=0.9):
    s1 = q1.get('question', '') + q1.get('answer', '')
    s2 = q2.get('question', '') + q2.get('answer', '')
    return SequenceMatcher(None, s1, s2).ratio() > threshold

def deduplicate_qa_pairs(qa_pairs, threshold=0.9):
    unique = []
    for qa in qa_pairs:
        if not any(is_similar(qa, u, threshold) for u in unique):
            unique.append(qa)
    return unique
